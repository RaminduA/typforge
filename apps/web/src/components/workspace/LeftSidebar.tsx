...
export function LeftSidebar({
  ...
  const [projectMenu, setProjectMenu] = useState<MenuPosition | null>(null);
  ...
  function openProjectMenu(event: MouseEvent<HTMLButtonElement>) {
    ...
    setProjectMenu({ x: bounds.left, y: bounds.bottom + 6 });
  }
  ...
  function openRootMenu(event: MouseEvent<HTMLButtonElement>) {
    ...
    const bounds = event.currentTarget.getBoundingClientRect();
    setRootMenu({ x: bounds.right - 4, y: bounds.bottom + 5 });
  }
  ...
  const projectMenuItems: PopupMenuItem[] = [
      {
        id: "share-project",
        label: "Share and collaborate",
        icon: (<Share2 size={15} />),
        onSelect: onShareProject
      },
      {
        id: "rename-project",
        label: "Rename",
    ...
    <aside className="sidebar">
      ...
      <div className="project-name-row">
        ...
        >
          ...
          {projectMenu ? (<ChevronUp size={17} />) : (<ChevronDown size={17} />)}
        </button>
